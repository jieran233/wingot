...
def winget_main():
    ...
    if winget.check_available():
        ...
        # list upgradable packages (synchronize package databases included)
        ...
        utils.printf('info', "WinGet unavailable, skipping\n" + sgr.f(
            "Install WinGet: https://learn.microsoft.com/en-us/windows/package-manager/winget/"))
        print()


def scoop_main():
    working_packing_manager = 'Scoop'
    if scoop.check_available():

        # clean up downloading caches & old versions
        utils.printf('title', "[{}] Cleaning up downloading caches & old versions...".format(working_packing_manager))
        print(scoop.cleanup())

        # synchronize package databases
        utils.printf('title', "[{}] Synchronizing package databases...".format(working_packing_manager))
        update = scoop.update()
        print(update)

        # list upgradable packages
        utils.printf('title', "[{}] Searching for updates...".format(working_packing_manager))
        upgradable = scoop.list_upgradable()
        print(upgradable)

        # list excluded package(s) (if exist)
        excluded = scoop.list_excluded()
        if scoop.list_excluded() is not None:
            excluded_exist = True
            utils.printf('sub_warning', "Excluded package(s):")
            print(excluded)
        else:
        ...
        if _add_excluded_pkgs is not None:
            add_excluded_pkgs = _add_excluded_pkgs.split(' ')
            scoop.managing_excluded('hold', add_excluded_pkgs)
        print()
        ...
        if _remove_excluded_pkgs is not None:
            remove_excluded_pkgs = _remove_excluded_pkgs.split(' ')
            scoop.managing_excluded('unhold', remove_excluded_pkgs)
        print()
        ...
        if utils.yes_or_no_ask("[{}] Proceed with installation?".format(working_packing_manager), default=True):
            scoop.fully_upgrading()

        print()

    else:
        utils.printf('info', "Scoop unavailable, skipping\n" + sgr.f(
